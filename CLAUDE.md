...
- **Input**: a single `keydown` listener (ignored before the first `init()` and while typing in a text `<input>`) handles arrows (move/soft-drop), Up/X (rotate), Space (hard drop, with `preventDefault`), and P/Escape (pause menu, ignoring `e.repeat`), gated by `paused`/`gameOver` flags.
